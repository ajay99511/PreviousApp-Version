...
public class LikeRepository(DataContext context, IMapper mapper) : ILikeRepository
{
    ...
    public async Task<IEnumerable<int>> GetCurrentUserLikeIds(int currentUserId)
    {
        return await context.Likes
            .Where(s=>s.SourceUserId == currentUserId)
    ...
    public async Task<PagedList<MemberDto>> GetUserLikes(LikesParams likesParams)
    {
        ...
        switch (likesParams.Predicate)
        {
            ...
            case "likedBy":
                ...
                break;
            default:
                var userLikeIds = await GetCurrentUserLikeIds(likesParams.UserId);
                query = likes.Where(x=>x.TargetUserId == likesParams.UserId && userLikeIds.Contains(x.SourceUserId))
